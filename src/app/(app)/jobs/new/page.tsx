...
import { ArrowLeft, Trash2, Wand2, Loader2, Sparkles, PackagePlus, AlertTriangle, Download, FileText } from "lucide-react";
...
import { Job, Company, UserProfile, Application } from "@/lib/data";
...
export default function NewJobPage() {
    ...
    useEffect(() => {
        ...
        if (duplicateJobId && jobs) {
            const jobToDuplicate = jobs.find(j => j.id === duplicateJobId);
            if (jobToDuplicate) {
                const { id, status, postedDate, ...restOfJob } = jobToDuplicate;
                
                const duplicatedValues: Partial<JobFormValues> = {
                    ...restOfJob,
                    title: `${restOfJob.title} (Copia)`,
                    slug: restOfJob.slug ? `${restOfJob.slug}-copia` : '',
                    responsibilities: restOfJob.responsibilities.map(value => ({ value })),
                    requirements: restOfJob.requirements.map(value => ({ value })),
                    competencies: restOfJob.competencies.map(value => ({ value })),
                    languages: restOfJob.languages,
                    benefits: restOfJob.benefits.map(value => ({ value })),
                    recruitmentStages: restOfJob.recruitmentStages?.map(value => ({ value })) || [],
                    assignedRecruiters: restOfJob.assignedRecruiters || [],
                    assignedInterviewers: restOfJob.assignedInterviewers || [],
                };
                
                form.reset(duplicatedValues);
                toast({
                    title: "Plaza Duplicada",
                    description: `Cargando datos de "${jobToDuplicate.title}".`,
                });
            }
        }
    }, [searchParams, form, toast, jobs]);
    ...
    const handleAutofill = async (description: string) => {
        ...
        try {
            ...
            if (result.responsibilities && result.responsibilities.length > 0) {
                form.setValue('responsibilities', result.responsibilities.map(r => ({ value: r.value })), { shouldValidate: true });
            }
            if (result.requirements && result.requirements.length > 0) {
                form.setValue('requirements', result.requirements.map(r => ({ value: r.value })), { shouldValidate: true });
            }
            if (result.competencies && result.competencies.length > 0) {
                form.setValue('competencies', result.competencies.map(c => ({ value: c.value })), { shouldValidate: true });
            }
            if (result.languages && result.languages.length > 0) {
                form.setValue('languages', result.languages.map(l => ({ name: l.name, level: l.level })), { shouldValidate: true });
            }
            if (result.benefits && result.benefits.length > 0) {
                form.setValue('benefits', result.benefits.map(b => ({ value: b.value })), { shouldValidate: true });
            }
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                ...
                setJobDescriptionToAnalyze(text);
                handleAutofill(text);
            };
    ...
    const handleGenerateImage = async () => {
        ...
        try {
            ...
            // const result = { imageUrl: `https://picsum.photos/seed/${Date.now()}/1200/400` };
            // form.setValue('coverImage', result.imageUrl, { shouldValidate: true });
            
            toast({
                title: 'Imagen Generada',
                description: 'La imagen de portada ha sido creada.',
            });
        } catch (error) {
    ...
    async function onSubmit(data: JobFormValues) {
        ...
        try {
            ...
            const jobDataToSave = {
                ...
                competencies: data.competencies.map(c => c.value),
            ...
            await addDoc(jobsCollection, jobDataToSave);

            toast({
                title: "Plaza Creada",
                description: "La nueva plaza de trabajo ha sido creada exitosamente.",
            });
            router.push(`/jobs`);
        } catch (error) {
            ...
            toast({
                ...
                description: "No se pudo crear la plaza de trabajo."
            });
        } finally {
            setIsSaving(false);
        }
    ...
    return (
        <>
            <DashboardHeader title="Crear Nueva Plaza" description="Completa los detalles de la nueva oferta de trabajo.">
                        ...
                        />
                         <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div className="flex items-center gap-4">
                                <Label htmlFor="jd-file" className="text-sm text-muted-foreground flex-shrink-0">O sube un archivo:</Label>
                                <Input 
                                    id="jd-file" 
                                    type="file"
                                    className="max-w-xs" 
                                ...
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Button variant="outline"><Download className="mr-2"/>Descargar Plantilla</Button>
                                    </DropdownMenuTrigger>
                                    <DropdownMenuContent>
                                        <DropdownMenuItem asChild>
                                            <a href="/plantilla_plaza.txt" download><FileText className="mr-2"/>Plantilla de Texto (.txt)</a>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem asChild>
                                            <a href="/plantilla_plaza.docx.txt" download="plantilla_plaza.docx"><FileText className="mr-2"/>Plantilla de Word (.docx)</a>
                                        </DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </div>
                            <Button onClick={() => handleAutofill(jobDescriptionToAnalyze)} disabled={isAutofilling} className="w-full sm:w-auto">
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                                    ...
                                    <FormControl>
                                        <Card className="overflow-hidden border-dashed border-2">
                                            <CardContent className="p-0">
                                                <div className="relative aspect-[16/5] w-full flex items-center justify-center bg-muted">
                                                    {isGeneratingImage ? (
                                                        ...
                                                        <p className="text-muted-foreground">Sube o genera una imagen</p>
                                                    )}
                                                </div>
                                            </CardContent>
                                        </Card>
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        <Button type="submit" disabled={companyConfigIncomplete || isSaving}>
